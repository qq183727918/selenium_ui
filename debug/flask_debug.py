...
@api.route('/index/logins', methods=['get'])
def logins():
    ren = {'msg': index(), 'msg_code': 200}
    # json.dumps 序列化时对中文默认使用的ascii编码.想输出中文需要指定ensure_ascii=False
    return json.dumps(ren, ensure_ascii=False)
...
@api.route('/liu', methods=['post'])
def liu():
    name = flask.request.values.get('name')
    passwd = flask.request.values.get('passwd')

    if name and passwd:
        if name == 'liu' and passwd == 'zheng':
            ren = {'aini': 'nice', 'msg_cde': 200}
        else:
            ren = {'sha': 'chun', 'msg_code': 10021}
    else:
        ren = {'msg': 'ben', 'msg_code': 10022}
