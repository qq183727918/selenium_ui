...
path = r'D:\Tools\git\selenium_ui\testcase\purchaseOrder'
file_path = r'D:\Tools\git\selenium_ui\report\purchaseOrder'

# 自动生成测试套件
suite = unittest.defaultTestLoader.discover(fr"{path}", pattern='*.py')

# 定义测试报告文件对象
file = open(fr'{file_path}\purchaseOrder{time.strftime("%Y-%m-%d")}.html', 'wb')
